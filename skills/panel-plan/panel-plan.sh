...
build_argv() {
  ...
  case "$name" in
    ...
    opencode)
      ...
      # concise. panelist_error_reason() greps these lines to report *why* a
      # panelist produced nothing.
      argv=(opencode run --agent "$OPENCODE_AGENT" --print-logs --log-level ERROR)
...
# string that a panelist merely *echoed* from the plan/repo under review.
...
print_section() {
  ...
  echo
  # Keep the `done (exit N)` token intact — the panel-plan and panel-plan-loop
  # skills poll stderr for it to mark a panelist complete, so renaming it on
  # failure would make the orchestrator wait forever. Append a FAILED suffix
  # (reason truncated) instead, so an empty/errored panelist is loud rather than
  # indistinguishable from a clean run.
  local hb="panel-plan: ${p} (${model_label}) done (exit ${rc_val})"
  if (( failed )); then
    local short="$reason"
