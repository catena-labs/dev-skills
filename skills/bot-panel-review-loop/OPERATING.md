...
`/loop` with no interval runs in _dynamic mode_ — the driver (the main agent)
...
| `/bot-panel-review-loop` (this skill) | ONE sweep: `select-prs.sh` to pick PRs, dispatch one sub-agent per PR, `pr-actions.sh` for all GitHub calls. Does NOT schedule; pins the panel via `--panelist` (claude + codex + claude/decompose). Owns the 3-concurrent cap (it is the dispatcher). |
...
1. Have the `codex` and `claude` CLIs installed, on `PATH`, and authenticated
   (and `gh` authenticated for the target repo). The skill pins the panel to
   `claude + codex + claude/decompose`, so opencode is excluded automatically —
   no `PANEL_REVIEW_PANELISTS` setup is needed.
2. `cd` into the target repo (e.g. `catena-labs/bank`).
3. Run `/loop /bot-panel-review-loop`.
4. Drive it per the operating model above: ~5-min sweeps, up to 3 concurrent
   reviews, skip in-flight PRs, watch for silent panel misses, handle stale-🚀
   on re-reviews.
